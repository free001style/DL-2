...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        self.fpn_params = nn.ModuleDict({
            "conv1x1_p3": nn.Conv2d(dummy_out_shapes[0][1][1], out_channels, 1),
            "conv1x1_p4": nn.Conv2d(dummy_out_shapes[1][1][1], out_channels, 1),
            "conv1x1_p5": nn.Conv2d(dummy_out_shapes[2][1][1], out_channels, 1),
            "conv3x3_p3": nn.Conv2d(out_channels, out_channels, 3, padding=1),
            "conv3x3_p4": nn.Conv2d(out_channels, out_channels, 3, padding=1),
            "conv3x3_p5": nn.Conv2d(out_channels, out_channels, 3, padding=1)
        })
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def forward(self, images: torch.Tensor):
        ...
        fpn_feats["p5"] = self.fpn_params["conv1x1_p5"](backbone_feats["c5"])
        fpn_feats["p4"] = F.interpolate(fpn_feats["p5"], scale_factor=2) + self.fpn_params["conv1x1_p4"](
            backbone_feats["c4"])
        fpn_feats["p3"] = F.interpolate(fpn_feats["p4"], scale_factor=self.fpn_strides["p4"] / self.fpn_strides["p3"]) + \
                          self.fpn_params["conv1x1_p3"](backbone_feats["c3"])

        fpn_feats["p5"] = self.fpn_params["conv3x3_p5"](fpn_feats["p5"])
        fpn_feats["p4"] = self.fpn_params["conv3x3_p4"](fpn_feats["p4"])
        fpn_feats["p3"] = self.fpn_params["conv3x3_p3"](fpn_feats["p3"])
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        ######################################################################
        # Replace "pass" statement with your code
        location_coords[level_name] = ((torch.cartesian_prod(torch.arange(feat_shape[2]), torch.arange(feat_shape[3])) + 0.5)\
              * strides_per_fpn_level[level_name]).to(device)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        stem_box = [
            nn.Conv2d(in_channels, stem_channels[1], 3, padding=1),
            nn.ReLU(),
        ...
        self.pred_cls = nn.Conv2d(stem_channels[0], num_classes, 3,
                                  padding=1)  # head cls
        self.pred_box = nn.Conv2d(stem_channels[1], 4, 3, padding=1)  # box head
        self.pred_ctr = nn.Conv2d(stem_channels[1], 1, 3, padding=1)  # ctr head
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key, fpn_level in feats_per_fpn_level.items():
            cls_out = self.stem_cls(fpn_level)
            box_out = self.stem_box(fpn_level)
            class_logits[key] = self.pred_cls(cls_out).flatten(-2).transpose(1, 2)
            boxreg_deltas[key] = self.pred_box(box_out).flatten(-2).transpose(1, 2)
            centerness_logits[key] = self.pred_ctr(box_out).flatten(-2).transpose(1, 2)
...
def fcos_get_deltas_from_locations(
    ...
    xc, yc = locations[..., :2].unbind(dim=-1)  # прекрасная вещь
    x0, y0, x1, y1 = gt_boxes[..., :4].unbind(dim=-1)
    l = (xc - x0) / stride
    r = (x1 - xc) / stride
    t = (yc - y0) / stride
    b = (y1 - yc) / stride
...
def fcos_apply_deltas_to_locations(
    ...
    ##########################################################################
    torch.clip_(deltas, 0)

    l, t, r, b = deltas[..., :4].unbind(dim=-1)
    xc, yc = locations[..., :2].unbind(dim=-1)
    x0 = xc - l * stride
    x1 = xc + r * stride
    y0 = yc - t * stride
    y1 = yc + b * stride
    output_boxes = torch.stack((x0, y0, x1, y1), dim=-1)
    output_boxes[l == -1] = torch.stack((xc, yc, xc, yc), dim=-1)[l == -1]
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def box_square(left, top, right, bottom):
    return (right - left) * (bottom - top)


def iou(box1: torch.Tensor, box2: torch.Tensor):
    left_i, top_i, right_i, bottom_i = box1.unbind()
    left_j, top_j, right_j, bottom_j = box2.unbind()
    ...
    return inter / (-inter + (right_i - left_i) * (bottom_i - top_i) + (right_j - left_j) * (bottom_j - top_j))
...
def nms(boxes: torch.Tensor, scores: torch.Tensor, iou_threshold: float = 0.5):
    ...
    for box_index in index:
        for keep_index in keep:
            if iou(boxes[keep_index], boxes[box_index]) > iou_threshold:
                break
        else:
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        ######################################################################
        # Feel free to delete this line: (but keep variable names same)
        shape_per_fpn_level = {key: value.shape for key, value in fpn_features.items()}
        ...
        matched_gt_boxes = [
            fcos_match_locations_to_gt(locations_per_fpn_level, strides_per_fpn_level, gt_boxes_per_image) for
            gt_boxes_per_image in gt_boxes]

        # Calculate GT deltas for these matched boxes. Similar structure
        # as `matched_gt_boxes` above. Fill this list:
        matched_gt_deltas = []

        for gt_boxes_dict in matched_gt_boxes:
            tmp = {}
            for level_name, gt_boxes_on_level in gt_boxes_dict.items():
        ...
        ######################################################################
        # Feel free to delete this line: (but keep variable names same)
        one_hot_gt = (matched_gt_boxes[..., None] == torch.arange(11, device=images.device)[:, None, :]).to(
            torch.float32)

        loss_cls = sigmoid_focal_loss(
            inputs=pred_cls_logits, targets=one_hot_gt
        )
        ...
        ctr_gt = fcos_make_centerness_targets(matched_gt_deltas)
        loss_ctr = F.binary_cross_entropy_with_logits(
            pred_ctr_logits.squeeze(-1), ctr_gt, reduction="none"
        )
        loss_ctr[ctr_gt < 0] *= 0.0
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Replace "pass" statement with your code
            level_pred_boxes = \
            fcos_apply_deltas_to_locations(level_deltas, level_locations, self.backbone.fpn_strides[level_name])[mask]

            # Step 4: Use `images` to get (height, width) for clipping.
            # Replace "pass" statement with your code
            level_pred_boxes[:, 0].clip_(0, images.shape[-1])
            level_pred_boxes[:, 1].clip_(0, images.shape[-2])
            level_pred_boxes[:, 2].clip_(0, images.shape[-1])
            level_pred_boxes[:, 3].clip_(0, images.shape[-2])
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
